...
CANADIAN_STORES = [
    ...
    },
    {
        ...
        "logo_url": "https://via.placeholder.com/48x48/E31837/FFFFFF?text=S",
        "brand_color": "#E31837"
    },
    {
...
def send_grocery_list_email(email: str, grocery_list_data: Dict[str, Any], user_name: Optional[str] = None):
    """Send grocery list via email using SendGrid"""
    try:
        # Extract grocery list details
        items = grocery_list_data.get('items', [])
        total_cost = grocery_list_data.get('total_cost', 0)
        total_savings = grocery_list_data.get('total_savings', 0)
        
        # Create email subject
        subject = f"Your SaleSavor Grocery List - ${total_cost:.2f} (Save ${total_savings:.2f}!)"
        ...
        <html>
            <head>
                <style>
                    ...
                    .header {{ background: linear-gradient(135deg, #0f766e, #14b8a6); color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }}
                    .content {{ padding: 20px; background: #f8fafc; }}
                    .summary {{ background: white; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #14b8a6; }}
                    .item {{ background: white; padding: 12px; margin: 8px 0; border-radius: 6px; border: 1px solid #e2e8f0; }}
                    .item-name {{ font-weight: bold; color: #1e293b; }}
                    .store-name {{ color: #64748b; font-size: 0.9em; }}
                    .price {{ float: right; font-weight: bold; }}
                    .sale-price {{ color: #059669; }}
            ...
            <body>
                <div class="header">
                    <h2>🛒 Your SaleSavor Grocery List</h2>
                    {"<p>Hi " + user_name + "!</p>" if user_name else ""}
                    <p>Bringing Family Meals Home - For Less</p>
                </div>
                
                <div class="content">
                    <div class="summary">
                        <h3>📊 Shopping Summary</h3>
                        <p><strong>Total Cost:</strong> <span style="color: #059669; font-size: 1.2em;">${total_cost:.2f}</span></p>
                        <p><strong>You Save:</strong> <span style="color: #dc2626; font-size: 1.2em;">${total_savings:.2f}</span></p>
                        <p><strong>Items:</strong> {len(items)}</p>
                    </div>
                    
                    <h3>🛍️ Your Grocery List</h3>
        """
        
        # Add each grocery item
        for item in items:
            sale_badge = '<span style="background: #dc2626; color: white; padding: 2px 6px; border-radius: 12px; font-size: 0.8em; margin-left: 8px;">SALE</span>' if item.get('is_on_sale') else ''
            
            html_content += f"""
                    <div class="item">
                        <div class="item-name">{item.get('ingredient', 'Unknown Item')}{sale_badge}</div>
                        <div class="store-name">📍 {item.get('store_name', 'Unknown Store')} • {item.get('quantity', 'Unknown quantity')}</div>
                        <div class="price">
                            {"<span class='sale-price'>${:.2f}</span>".format(item.get('sale_price', item.get('price', 0))) if item.get('is_on_sale') else "${:.2f}".format(item.get('price', 0))}
                            {"<br><span class='original-price'>${:.2f}</span>".format(item.get('price', 0)) if item.get('is_on_sale') and item.get('sale_price') else ''}
                        </div>
        ...
        html_content += f"""
                ...
                <div class="footer">
                    <p>💡 <strong>Tip:</strong> Show this list on your phone while shopping!</p>
                    <p>Happy shopping and happy saving! 🎉</p>
                    <p style="font-size: 0.8em; color: #9ca3af;">
                        This list was generated by SaleSavor - Your smart grocery savings companion<br>
        ...
        message = Mail(
            from_email=os.environ.get('SENDER_EMAIL', 'salesavor@yourdomain.com'),
            to_emails=email,
            subject=subject,
        ...
        if not api_key or api_key == 'your_sendgrid_api_key_here':
            ...
            logging.info(f"Email would be sent to {email} with subject: {subject}")
            return True
        ...
        logging.error(f"Error sending grocery list email: {str(e)}")
        raise EmailDeliveryError(f"Failed to send email: {str(e)}")

@api_router.post("/email-grocery-list")
async def email_grocery_list(request: EmailGroceryListRequest, background_tasks: BackgroundTasks):
    """Email grocery list to user"""
    try:
        # Add email sending to background tasks for better performance
        background_tasks.add_task(
            send_grocery_list_email,
            request.email,
            request.grocery_list_data,
            request.user_name
        )
        
        return {
            "status": "success", 
            "message": f"Grocery list has been emailed to {request.email}"
        }
